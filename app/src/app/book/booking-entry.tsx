...
export function BookingEntry({ initialDepartment }: { initialDepartment?: string }) {
      ...
      <div>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {departments.map((department) => (
            <button key={department.slug} type="button" onClick={() => router.push(`/book/service?department=${department.slug}`)} className="min-h-11 rounded-full border bg-card px-4 py-2 text-sm font-medium hover:border-primary hover:text-primary">
              {department.name.en}
            </button>
          ))}
        </div>
